...
def geometry(l, n_l, r_e, r_i, d, alpha_mesh=1, with_cylinder=True):
    ...
    if with_cylinder:
        ...
        ph_cylinder_ends = add_physical_group(
            2, [cylinder_bottom_end, cylinder_top_end], "cylinder_ends"
        )
    ...
    surfs.append(
        get_cylinder_boundary(2, air, 10 * l, 2 * l_ends + 11 * l, -5 * l - l_ends)
    )
...
def sif(
    ...
    if with_cylinder:
        if mgdyn:
            eqn_mgdyn_heat = elmer.Equation(
                sim, "mgdyn_heat", [solver_mgdyn, solver_calcfields, solver_heat]
            )
        else:
            eqn_statmag_heat = elmer.Equation(
                sim, "mgdyn_heat", [solver_statmag, solver_heat]
            )
...
def run_elmer():
    ...
    with open("./simdata/elmergrid.log", "w") as f:
        args = [
            "C:/Program Files/Elmer 8.4-Release/bin/ElmerGrid.exe",
            "14",
            "2",
            "./simdata/induction_verification.msh2",
        ]
